export const vocabulary = [
    {
        "id":"tp4_v_1",
        "type":"vocabulary",
        "moji":"あいます",
        "romaji":"aimasu",
        "english":"to meet",
        "audio":"t4_aimasu.mp3",
    },
    {
        "id":"tp4_v_2",
        "type":"vocabulary",
        "moji":"あそびます",
        "romaji":"asobimasu",
        "english":"play",
        "audio":"t4_asobimasu.mp3",
    },
    {
        "id":"tp4_v_3",
        "type":"vocabulary",
        "moji":"あたたかい",
        "romaji":"atatakai",
        "english":"warm",
        "audio":"t4_atatakai.mp3",
    },
    {
        "id":"tp4_v_4",
        "type":"vocabulary",
        "moji":"いたい",
        "romaji":"itai",
        "english":"painful",
        "audio":"t4_itai.mp3",
    },
    {
        "id":"tp4_v_5",
        "type":"vocabulary",
        "moji":"いっしょ",
        "romaji":"issho",
        "english":"together",
        "audio":"t4_issho.mp3",
    },
    {
        "id":"tp4_v_6",
        "type":"vocabulary",
        "moji":"いつも",
        "romaji":"itsu mo",
        "english":"always",
        "audio":"t4_itsumo.mp3",
    },
    {
        "id":"tp4_v_7",
        "type":"vocabulary",
        "moji":"うみ",
        "romaji":"umi",
        "english":"sea",
        "audio":"t4_umi.mp3",
    },
    {
        "id":"tp4_v_8",
        "type":"vocabulary",
        "moji":"おかね",
        "romaji":"o kane",
        "english":"money",
        "audio":"t4_okane.mp3",
    },
    {
        "id":"tp4_v_9",
        "type":"vocabulary",
        "moji":"おなか",
        "romaji":"onaka",
        "english":"stomach",
        "audio":"t4_onaka.mp3",
    },
    {
        "id":"tp4_v_10",
        "type":"vocabulary",
        "moji":"かいもの",
        "romaji":"kaimono",
        "english":"Shopping",
        "audio":"t4_kaimono.mp3",
    },
    {
        "id":"tp4_v_11",
        "type":"vocabulary",
        "moji":"かかります",
        "romaji":"kakarimasu",
        "english":"to take time or money",
        "audio":"t4_kakarimasu.mp3",
    },
    {
        "id":"tp4_v_12",
        "type":"vocabulary",
        "moji":"～かげつ",
        "romaji":"～ ka getsu",
        "english":"~month(s)",
        "audio":"t4_kagetsu.mp3",
    },
    {
        "id":"tp4_v_13",
        "type":"vocabulary",
        "moji":"かけます",
        "romaji":"kakemasu",
        "english":"to call by phone",
        "audio":"t4_kakemasu.mp3",
    },
    {
        "id":"tp4_v_14",
        "type":"vocabulary",
        "moji":"かります",
        "romaji":"karimasu",
        "english":"to borrow",
        "audio":"t4_karimasu.mp3",
    },
    {
        "id":"tp4_v_15",
        "type":"vocabulary",
        "moji":"きっさてん",
        "romaji":"kissaten",
        "english":"coffee lounge",
        "audio":"t4_kissaten.mp3",
    },
    {
        "id":"tp4_v_16",
        "type":"vocabulary",
        "moji":"きっぷ",
        "romaji":"kippu",
        "english":"Ticket",
        "audio":"t4_kippu.mp3",
    },
    {
        "id":"tp4_v_17",
        "type":"vocabulary",
        "moji":"きます",
        "romaji":"kimasu",
        "english":"to put on from the shoulders down",
        "audio":"t4_kimasu.mp3",
    },
    {
        "id":"tp4_v_18",
        "type":"vocabulary",
        "moji":"くもり",
        "romaji":"kumori",
        "english":"cloudy weather",
        "audio":"t4_kumori.mp3",
    },
    {
        "id":"tp4_v_19",
        "type":"vocabulary",
        "moji":"げんき",
        "romaji":"genki",
        "english":"health, vitality",
        "audio":"t4_genki.mp3",
    },
    {
        "id":"tp4_v_20",
        "type":"vocabulary",
        "moji":"ことし",
        "romaji":"kotoshi",
        "english":"this year",
        "audio":"t4_kotoshi.mp3",
    },
    {
        "id":"tp4_v_21",
        "type":"vocabulary",
        "moji":"こんげつ",
        "romaji":"kon getsu",
        "english":"this month",
        "audio":"t4_kongetsu.mp3",
    },
    {
        "id":"tp4_v_22",
        "type":"vocabulary",
        "moji":"こんしゅう",
        "romaji":"kon Shū",
        "english":"this week",
        "audio":"t4_konshuu.mp3",
    },
    {
        "id":"tp4_v_23",
        "type":"vocabulary",
        "moji":"こんばん",
        "romaji":"konban",
        "english":"this evening",
        "audio":"t4_konban.mp3",
    },
    {
        "id":"tp4_v_24",
        "type":"vocabulary",
        "moji":"さんぽします",
        "romaji":"sanpo shimasu",
        "english":"to stroll",
        "audio":"t4_sanposhimasu.mp3",
    },
    {
        "id":"tp4_v_25",
        "type":"vocabulary",
        "moji":"～しゅうかん",
        "romaji":"～ shūkan",
        "english":"~week(s)",
        "audio":"t4_shuukan.mp3",
    },
    {
        "id":"tp4_v_26",
        "type":"vocabulary",
        "moji":"すずしい",
        "romaji":"suzushī",
        "english":"Refreshing",
        "audio":"t4_suzushii.mp3",
    },
    {
        "id":"tp4_v_27",
        "type":"vocabulary",
        "moji":"せんげつ",
        "romaji":"sengetsu",
        "english":"last month",
        "audio":"t4_sengetsu.mp3",
    },
    {
        "id":"tp4_v_28",
        "type":"vocabulary",
        "moji":"せんしゅう",
        "romaji":"senshū",
        "english":"last week",
        "audio":"t4_senshuu.mp3",
    },
    {
        "id":"tp4_v_29",
        "type":"vocabulary",
        "moji":"たいへん",
        "romaji":"taihen",
        "english":"hard",
        "audio":"t4_taihen.mp3",
    },
    {
        "id":"tp4_v_30",
        "type":"vocabulary",
        "moji":"たのしい",
        "romaji":"tanoshī",
        "english":"enjoyable",
        "audio":"t4_tanoshii.mp3",
    },
    {
        "id":"tp4_v_31",
        "type":"vocabulary",
        "moji":"ちず",
        "romaji":"chizu",
        "english":"map",
        "audio":"t4_chizu.mp3",
    },
    {
        "id":"tp4_v_32",
        "type":"vocabulary",
        "moji":"つかいます",
        "romaji":"tsukaimasu",
        "english":"to use",
        "audio":"t4_tsukaimasu.mp3",
    },
    {
        "id":"tp4_v_33",
        "type":"vocabulary",
        "moji":"てんき",
        "romaji":"tenki",
        "english":"weather",
        "audio":"t4_tenki.mp3",
    },
    {
        "id":"tp4_v_34",
        "type":"vocabulary",
        "moji":"どうやって",
        "romaji":"dō yatte",
        "english":"in what way",
        "audio":"t4_douyatte.mp3",
    },
    {
        "id":"tp4_v_35",
        "type":"vocabulary",
        "moji":"どうして",
        "romaji":"dō shite",
        "english":"why",
        "audio":"t4_doushite.mp3",
    },
    {
        "id":"tp4_v_36",
        "type":"vocabulary",
        "moji":"ときどき",
        "romaji":"tokidoki",
        "english":"sometimes",
        "audio":"t4_tokidoki.mp3",
    },
    {
        "id":"tp4_v_37",
        "type":"vocabulary",
        "moji":"どのぐらい",
        "romaji":"dono gurai",
        "english":"how long",
        "audio":"t4_donogurai.mp3",
    },
    {
        "id":"tp4_v_38",
        "type":"vocabulary",
        "moji":"パーティー",
        "romaji":"pātī",
        "english":"party",
        "audio":"t4_pati.mp3",
    },
    {
        "id":"tp4_v_39",
        "type":"vocabulary",
        "moji":"はたらきます",
        "romaji":"hatarakimasu",
        "english":"to work",
        "audio":"t4_hatarakimasu.mp3",
    },
    {
        "id":"tp4_v_40",
        "type":"vocabulary",
        "moji":"はなします",
        "romaji":"hanashimasu",
        "english":"to speak",
        "audio":"t4_hanashimasu.mp3",
    },
    {
        "id":"tp4_v_41",
        "type":"vocabulary",
        "moji":"ビール",
        "romaji":"bīru",
        "english":"beer",
        "audio":"t4_biru.mp3",
    },
    {
        "id":"tp4_v_42",
        "type":"vocabulary",
        "moji":"ひこうき",
        "romaji":"hi kōki",
        "english":"airplane",
        "audio":"t4_hikouki.mp3",
    },
    {
        "id":"tp4_v_43",
        "type":"vocabulary",
        "moji":"ひま",
        "romaji":"hima",
        "english":"free time",
        "audio":"t4_hima.mp3",
    },
    {
        "id":"tp4_v_44",
        "type":"vocabulary",
        "moji":"プール",
        "romaji":"pūru",
        "english":"swimming pool",
        "audio":"t4_puuru.mp3",
    },
    {
        "id":"tp4_v_45",
        "type":"vocabulary",
        "moji":"ふゆ",
        "romaji":"fuyu",
        "english":"winter",
        "audio":"t4_fuyu.mp3",
    },
    {
        "id":"tp4_v_46",
        "type":"vocabulary",
        "moji":"ホテル",
        "romaji":"hoteru",
        "english":"hotel",
        "audio":"t4_hoteru.mp3",
    },
    {
        "id":"tp4_v_47",
        "type":"vocabulary",
        "moji":"まいあさ",
        "romaji":"ma i asa",
        "english":"every morning",
        "audio":"t4_maiasa.mp3",
    },
    {
        "id":"tp4_v_48",
        "type":"vocabulary",
        "moji":"まいしゅう",
        "romaji":"ma i shū",
        "english":"every week",
        "audio":"t4_maishuu.mp3",
    },
    {
        "id":"tp4_v_49",
        "type":"vocabulary",
        "moji":"まいつき",
        "romaji":"ma itsuki",
        "english":"every month",
        "audio":"t4_maitsuki.mp3",
    },
    {
        "id":"tp4_v_50",
        "type":"vocabulary",
        "moji":"まいとし",
        "romaji":"mai to shi",
        "english":"every year",
        "audio":"t4_maitoshi.mp3",
    },
    {
        "id":"tp4_v_51",
        "type":"vocabulary",
        "moji":"まいばん",
        "romaji":"maiban",
        "english":"every night",
        "audio":"t4_maiban.mp3",
    },
    {
        "id":"tp4_v_52",
        "type":"vocabulary",
        "moji":"みんな",
        "romaji":"minna",
        "english":"everyone",
        "audio":"t4_minna.mp3",
    },
    {
        "id":"tp4_v_53",
        "type":"vocabulary",
        "moji":"やさい",
        "romaji":"yasai",
        "english":"vegetable",
        "audio":"t4_yasai.mp3",
    },
    {
        "id":"tp4_v_54",
        "type":"vocabulary",
        "moji":"ゆき",
        "romaji":"Yuki",
        "english":"snow",
        "audio":"t4_yuki.mp3",
    },
    {
        "id":"tp4_v_55",
        "type":"vocabulary",
        "moji":"よく",
        "romaji":"yoku",
        "english":"often, well",
        "audio":"t4_yoku.mp3",
    },
    {
        "id":"tp4_v_56",
        "type":"vocabulary",
        "moji":"らいげつ",
        "romaji":"rai getsu",
        "english":"next month",
        "audio":"t4_raigetsu.mp3",
    },
    {
        "id":"tp4_v_57",
        "type":"vocabulary",
        "moji":"らいしゅう",
        "romaji":"raishū",
        "english":"next week",
        "audio":"t4_raishuu.mp3",
    },
    {
        "id":"tp4_v_58",
        "type":"vocabulary",
        "moji":"らいねん",
        "romaji":"rai nen",
        "english":"next year",
        "audio":"t4_rainen.mp3",
    },
    {
        "id":"tp4_v_59",
        "type":"vocabulary",
        "moji":"りょこう",
        "romaji":"ryokō",
        "english":"travel",
        "audio":"t4_ryokou.mp3",
    },
]

export const kanji = [
    {
        "id":"tp4_k_1",
        "type":"kanji",
        "romaji":"jin, ni n/ hito",
        "moji":"じん、にん/ひと",
        "kanji":"人",
        "english":"person",
        "audio":"t4_jin.mp3"
    },
    {
        "id":"tp4_k_2",
        "type":"kanji",
        "romaji":"otoko",
        "moji":"おとこ",
        "kanji":"男",
        "english":"man",
        "audio":"t4_otoko.mp3"
    },
    {
        "id":"tp4_k_3",
        "type":"kanji",
        "romaji":"onna",
        "moji":"おんな",
        "kanji":"女",
        "english":"woman",
        "audio":"t4_onna.mp3"
    },
    {
        "id":"tp4_k_4",
        "type":"kanji",
        "romaji":"ko",
        "moji":"こ",
        "kanji":"子",
        "english":"child",
        "audio":"t4_ko.mp3"
    },
    {
        "id":"tp4_k_5",
        "type":"kanji",
        "romaji":"sen/ Saki",
        "moji":"せん/さき",
        "kanji":"先",
        "english":"ahead",
        "audio":"t4_saki.mp3"
    },
    {
        "id":"tp4_k_6",
        "type":"kanji",
        "romaji":"sei",
        "moji":"せい",
        "kanji":"生",
        "english":"birth",
        "audio":"t4_sei.mp3"
    },
    {
        "id":"tp4_k_7",
        "type":"kanji",
        "romaji":"tomo",
        "moji":"とも",
        "kanji":"友",
        "english":"friend",
        "audio":"t4_tomo.mp3"
    },
    {
        "id":"tp4_k_8",
        "type":"kanji",
        "romaji":"gaku/ ga",
        "moji":"がく/がっ",
        "kanji":"学",
        "english":"learning",
        "audio":"t4_gaku.mp3"
    },
    {
        "id":"tp4_k_9",
        "type":"kanji",
        "romaji":"kō",
        "moji":"こう",
        "kanji":"校",
        "english":"school",
        "audio":"t4_kou.mp3"
    },
    {
        "id":"tp4_k_10",
        "type":"kanji",
        "romaji":"ka i/ a",
        "moji":"かい/あ",
        "kanji":"会",
        "english":"to meet",
        "audio":"t4_kai.mp3"
    },
    {
        "id":"tp4_k_11",
        "type":"kanji",
        "romaji":"sha",
        "moji":"しゃ",
        "kanji":"社",
        "english":"company",
        "audio":"t4_sha.mp3"
    },
    {
        "id":"tp4_k_12",
        "type":"kanji",
        "romaji":"yama",
        "moji":"やま",
        "kanji":"山",
        "english":"mountain",
        "audio":"t4_yama.mp3"
    },
    {
        "id":"tp4_k_13",
        "type":"kanji",
        "romaji":"kawa",
        "moji":"かわ",
        "kanji":"川",
        "english":"river",
        "audio":"t4_kawa.mp3"
    },
    {
        "id":"tp4_k_14",
        "type":"kanji",
        "romaji":"hana",
        "moji":"はな",
        "kanji":"花",
        "english":"flower",
        "audio":"t4_hana.mp3"
    },
    {
        "id":"tp4_k_15",
        "type":"kanji",
        "romaji":"ka, ie",
        "moji":"か、いえ",
        "kanji":"家",
        "english":"house",
        "audio":"t4_ka.mp3"
    },
]

export const grammar = [
    {
        "id":"tp4_g_1",
        "type":"grammar",
        "romaji":"kissa-ten e kōhī o nomi ni ikimasu.",
        "moji":"喫茶店 へ コーヒー を 飲み に 行きます。",
        "kanji":"喫茶店 へ コーヒー を 飲み に 行きます。",
        "english":"I will go to coffee shop to drink a coffee.",
        "audio":"tp4_g_1.mp3"
    },
    {
        "id":"tp4_g_2",
        "type":"grammar",
        "romaji":"sūpā e yasai o kai ni ikimasu.",
        "moji":"スーパー へ 野菜 を 買い に 行きます。",
        "kanji":"スーパー へ 野菜 を 買い に 行きます。",
        "english":"I will go to supermarket to buy vegetable.",
        "audio":"tp4_g_2.mp3"
    },
    {
        "id":"tp4_g_3",
        "type":"grammar",
        "romaji":"tosho-kan e hon o kari ni ikimasu.",
        "moji":"図書館 へ 本 を 借り に 行きます。",
        "kanji":"図書館 へ 本 を 借り に 行きます。",
        "english":"I will go to library to borrow books.",
        "audio":"tp4_g_3.mp3"
    },
    {
        "id":"tp4_g_4",
        "type":"grammar",
        "romaji":"Amerika e eigo no benkyō ni ikimasu.",
        "moji":"アメリカ へ 英語 の 勉強 に 行きます。",
        "kanji":"アメリカ へ 英語 の 勉強 に 行きます。",
        "english":"I will go to America to study English.",
        "audio":"tp4_g_4.mp3"
    },
    {
        "id":"tp4_g_5",
        "type":"grammar",
        "romaji":"Tōkyō e kaimono ni ikimasu.",
        "moji":"東京 へ 買い物 に 行きます。",
        "kanji":"東京 へ 買い物 に 行きます。",
        "english":"I will go to Tokyo for shopping." ,
        "audio":"tp4_g_5.mp3"
     },
    {
        "id":"tp4_g_6",
        "type":"grammar",
        "romaji":"depāto e kaimono ni ikimasen ka.",
        "moji":"デパート へ 買い物 に 行きません か。",
        "kanji":"デパート へ 買い物 に 行きません か。",
        "english":"Won't you go to department store to shopping with me?",
        "audio":"tp4_g_6.mp3"
    },
    {
        "id":"tp4_g_7",
        "type":"grammar",
        "romaji":"o sake o nomi ni ikimasen ka.",
        "moji":"お酒 を 飲み に 行きません か。",
        "kanji":"お酒 を 飲み に 行きません か。",
        "english":"Won't you drink alcohol with me?",
        "audio":"tp4_g_7.mp3"
    },
    {
        "id":"tp4_g_8",
        "type":"grammar",
        "romaji":"issho ni benkyō shimasen ka.",
        "moji":"いっしょに 勉強 しません か。",
        "kanji":"いっしょに 勉強 しません か。",
        "english":"Won't you study with me?",
        "audio":"tp4_g_8.mp3"
    },
    {
        "id":"tp4_g_9",
        "type":"grammar",
        "romaji":"kono eiga o mimasen ka.",
        "moji":"この 映画 を 見ません か。",
        "kanji":"この 映画 を 見ません か。",
        "english":"Won't you watch this movie with me?",
        "audio":"tp4_g_9.mp3"
    },
    {
        "id":"tp4_g_10",
        "type":"grammar",
        "romaji":"issho ni kaerimasen ka.",
        "moji":"いっしょに 帰りません か。",
        "kanji":"いっしょに 帰りません か。",
        "english":"Won't you go home with me?",
        "audio":"tp4_g_10.mp3"
    },
    {
        "id":"tp4_g_11",
        "type":"grammar",
        "romaji":"tosho-kan de benkyō shimashō.",
        "moji":"図書館 で 勉強 しましょう。",
        "kanji":"図書館 で 勉強 しましょう。",
        "english":"Let's study at the library.",
        "audio":"tp4_g_11.mp3"
    },
    {
        "id":"tp4_g_12",
        "type":"grammar",
        "romaji":"kissa-ten de kōhī o nomimashō.",
        "moji":"喫茶店 で コーヒー を 飲み ましょう。",
        "kanji":"喫茶店 で コーヒー を 飲み ましょう。",
        "english":"Let's drink coffee at a coffee shop.",
        "audio":"tp4_g_12.mp3"
    },
    {
        "id":"tp4_g_13",
        "type":"grammar",
        "romaji":"basu de gakkō e ikimashō.",
        "moji":"バス で 学校 へ 行き ましょう。",
        "kanji":"バス で 学校 へ 行き ましょう。",
        "english":"Let's go to school by bus.",
        "audio":"tp4_g_13.mp3"
    },
    {
        "id":"tp4_g_14",
        "type":"grammar",
        "romaji":"uchi de bangohan o tsukurimashō.",
        "moji":"うち で 晩ご飯 を 作り ましょう。",
        "kanji":"うち で 晩ご飯 を 作り ましょう。",
        "english":"Let's make dinner at house.",
        "audio":"tp4_g_14.mp3"
    },
    {
        "id":"tp4_g_15",
        "type":"grammar",
        "romaji":"kaerimashō.",
        "moji":"帰り ましょう。",
        "kanji":"帰り ましょう。",
        "english":"Let's go home.",
        "audio":"tp4_g_15.mp3"
    },
    {
        "id":"tp4_g_16",
        "type":"grammar",
        "romaji":"ichi shūkan ni1 kai, sakkā o shimasu.",
        "moji":"一週間 に 1回、 サッカー を します。",
        "kanji":"一週間 に 1回、 サッカー を します。",
        "english":"I play football once a week.",
        "audio":"tp4_g_16.mp3"
    },
    {
        "id":"tp4_g_17",
        "type":"grammar",
        "romaji":"ichi shūkan ni3 kai, tomodachi to o sake o nomimasu.",
        "moji":"一週間 に 3回、 友達 と お酒 を 飲みます。",
        "kanji":"一週間 に 3回、 友達 と お酒 を 飲みます。",
        "english":"I drink alcohol with friend three times a week.",
        "audio":"tp4_g_17.mp3"
    },
    {
        "id":"tp4_g_18",
        "type":"grammar",
        "romaji":"ichi nen ni2 kai, kuni e kaerimasu.",
        "moji":"一年 に 2回、 国 へ 帰ります。",
        "kanji":"一年 に 2回、 国 へ 帰ります。",
        "english":"I return to my country twice a year.",
        "audio":"tp4_g_18.mp3"
    },
    {
        "id":"tp4_g_19",
        "type":"grammar",
        "romaji":"ichi nen ni3 kai, kazoku ni aimasu.",
        "moji":"一年 に 3回、 家族 に 会います。",
        "kanji":"一年 に 3回、 家族 に 会います。",
        "english":"I meet my family three times a year.",
        "audio":"tp4_g_19.mp3"
    },
    {
        "id":"tp4_g_20",
        "type":"grammar",
        "romaji":"ichi kagetsu ni3 kai, depāto e ikimasu.",
        "moji":"一か月 に 3回、 デパート へ 行きます。",
        "kanji":"一か月 に 3回、 デパート へ 行きます。",
        "english":"I go to department store three times a month.",
        "audio":"tp4_g_20.mp3"
    },
    {
        "id":"tp4_g_21",
        "type":"grammar",
        "romaji":"ichi kagetsu ni2 kai, Tōkyō e kaimono ni ikimasu.",
        "moji":"一か月 に 2回、 東京 へ 買い物 に 行きます。",
        "kanji":"一か月 に 2回、 東京 へ 買い物 に 行きます。",
        "english":"I go to Tokyo to shopping twice a month.",
        "audio":"tp4_g_21.mp3"
    },
    {
        "id":"tp4_g_22",
        "type":"grammar",
        "romaji":"kazoku ni aitaidesu.",
        "moji":"家族 に 会いたい です。",
        "kanji":"家族 に 会いたい です。",
        "english":"I want to meet my family.",
        "audio":"tp4_g_22.mp3"
    },
    {
        "id":"tp4_g_23",
        "type":"grammar",
        "romaji":"eiga o mitaidesu.",
        "moji":"映画 を 見たい です。",
        "kanji":"映画 を 見たい です。",
        "english":"I want to watch a movie.",
        "audio":"tp4_g_23.mp3"
    },
    {
        "id":"tp4_g_24",
        "type":"grammar",
        "romaji":"Nippon e ikitaidesu.",
        "moji":"日本 へ 行きたい です。",
        "kanji":"日本 へ 行きたい です。",
        "english":"I want to go to Japan.",
        "audio":"tp4_g_24.mp3"
    },
    {
        "id":"tp4_g_25",
        "type":"grammar",
        "romaji":"ie e kaeritaidesu.",
        "moji":"家 へ 帰りたい です。",
        "kanji":"家 へ 帰りたい です。",
        "english":"I want to go home.",
        "audio":"tp4_g_25.mp3"
    },
    {
        "id":"tp4_g_26",
        "type":"grammar",
        "romaji":"o sake o nomitaidesu.",
        "moji":"お酒 を 飲みたい です。",
        "kanji":"お酒 を 飲みたい です。",
        "english":"I want to drink alcohol.",
        "audio":"tp4_g_26.mp3"
    },
    {
        "id":"tp4_g_27",
        "type":"grammar",
        "romaji":"kinō no pātī wa tanoshikatta desu.",
        "moji":"きのう の パーティー は 楽しかった です。",
        "kanji":"きのう の パーティー は 楽しかった です。",
        "english":"The party was fun yesterday.",
        "audio":"tp4_g_27.mp3"
    },
    {
        "id":"tp4_g_28",
        "type":"grammar",
        "romaji":"senshū wa tenki ga yokatta desu.",
        "moji":"先週 は 天気 が よかった です。",
        "kanji":"先週 は 天気 が よかった です。",
        "english":"It was nice weather last week.",
        "audio":"tp4_g_28.mp3"
    },
    {
        "id":"tp4_g_29",
        "type":"grammar",
        "romaji":"kinō wa samuku nakatta desu.",
        "moji":"きのう は 寒くな かった です。",
        "kanji":"きのう は 寒くな かった です。",
        "english":"It was not cold yesterday.",
        "audio":"tp4_g_29.mp3"
    },
    {
        "id":"tp4_g_30",
        "type":"grammar",
        "romaji":"kinō wa isogashiku nakatta desu.",
        "moji":"きのう は 忙しくな かった です。",
        "kanji":"きのう は 忙しくな かった です。",
        "english":"It was not busy yesterday.",
        "audio":"tp4_g_30.mp3"
    },
    {
        "id":"tp4_g_31",
        "type":"grammar",
        "romaji":"Okinawa no umi wa kirei deshi ta.",
        "moji":"沖縄 の 海 は きれい でした。",
        "kanji":"沖縄 の 海 は きれい でした。",
        "english":"The sea was beautiful in Okinawa.",
        "audio":"tp4_g_31.mp3"
    },
    {
        "id":"tp4_g_32",
        "type":"grammar",
        "romaji":"kinō no shigoto wa taihen deshi ta.",
        "moji":"きのう の 仕事 は 大変 でした。",
        "kanji":"きのう の 仕事 は 大変 でした。",
        "english":"It was hard work yesterday.",
        "audio":"tp4_g_32.mp3"
    },
    {
        "id":"tp4_g_33",
        "type":"grammar",
        "romaji":"chichi wa genki ja arimasendeshita.",
        "moji":"父 は 元気 じゃ ありません でした。",
        "kanji":"父 は 元気 じゃ ありません でした。",
        "english":"My father was not healthy.",
        "audio":"tp4_g_33.mp3"
    },
    {
        "id":"tp4_g_34",
        "type":"grammar",
        "romaji":"kinō wa nichiyō hi deshi ta.",
        "moji":"きのう は 日曜日 でした。",
        "kanji":"きのう は 日曜日 でした。",
        "english":"Yesterday was Sunday.",
        "audio":"tp4_g_34.mp3"
    },
    {
        "id":"tp4_g_35",
        "type":"grammar",
        "romaji":"kinō wa ame deshi ta.",
        "moji":"きのう は 雨 でした。",
        "kanji":"きのう は 雨 でした。",
        "english":"It was rainy yesterday.",
        "audio":"tp4_g_35.mp3"
    },
    {
        "id":"tp4_g_36",
        "type":"grammar",
        "romaji":"kinō wa yuki ja arimasendeshita.",
        "moji":"きのう は 雪 じゃ ありません でした。",
        "kanji":"きのう は 雪 じゃ ありません でした。",
        "english":"It was not snow yesterday.",
        "audio":"tp4_g_36.mp3"
    },
    {
        "id":"tp4_g_37",
        "type":"grammar",
        "romaji":"takusan kaimono shimashita kara, o kane ga arimasen.",
        "moji":"たくさん 買い物 しました から、 お金 が ありません。",
        "kanji":"たくさん 買い物 しました から、 お金 が ありません。",
        "english":"Because I bought many things, I have no money.",
        "audio":"tp4_g_37.mp3"
    },
    {
        "id":"tp4_g_38",
        "type":"grammar",
        "romaji":"kinō wa samukatta desu kara, doko mo ikimasendeshita.",
        "moji":"きのう は 寒かった です から、 どこ も 行きません でした。",
        "kanji":"きのう は 寒かった です から、 どこ も 行きません でした。",
        "english":"Because it was cold yesterday, I didn't go anywhere.",
        "audio":"tp4_g_38.mp3"
    },
    {
        "id":"tp4_g_39",
        "type":"grammar",
        "romaji":"eigo o benkyō shitaidesu kara, Amerika e ikitaidesu.",
        "moji":"英語 を 勉強したい です から、 アメリカ へ 行きたい です。",
        "kanji":"英語 を 勉強したい です から、 アメリカ へ 行きたい です。",
        "english":"Because I want to study English, I want to go to America,",
        "audio":"tp4_g_39.mp3"
    },
    {
        "id":"tp4_g_40",
        "type":"grammar",
        "romaji":"raigetsu ryokō ni ikimasu kara, ōkī kaban o kaimasu.",
        "moji":"来月旅行 に 行きます から、 大きい かばん を 買います。",
        "kanji":"来月旅行 に 行きます から、 大きい かばん を 買います。",
        "english":"Because I travel next month,I will buy a big bag.",
        "audio":"tp4_g_40.mp3"
    },
    {
        "id":"tp4_g_41",
        "type":"grammar",
        "romaji":"ī tenki desu kara, asobi ni ikimasu.",
        "moji":"いい 天気 です から、 遊び に 行きます。",
        "kanji":"いい 天気 です から、 遊び に 行きます。",
        "english":"Because it is nice weather, I will go somewhere.",
        "audio":"tp4_g_41.mp3"
    },
]

export const listening = [
        {
        "id":"tp4_l_1",
        "type":"listening",
        "romaji":"喫茶店でコーヒーを飲みました",
        "moji":"喫茶店でコーヒーを飲みました",
        "kanji":"喫茶店でコーヒーを飲みました",
        "english":"喫茶店でコーヒーを飲みました",
        "answerOption":[
            {
                "id":"tp4_l_2",
                "type":"listening",
                "moji":"図書館で本を借りました"
            },
            {
                "id":"tp4_l_3",
                "type":"listening",
                "moji":"レストランで本を読みました"
            },
            {
                "id":"tp4_l_1",
                "type":"listening",
                "moji":"喫茶店でコーヒーを飲みました"
            },
            {
                "id":"tp4_l_4",
                "type":"listening",
                "moji":"デパートで本を買いました"
            }
        ],
        "audio":"t4_listening_1.mp3"
    },
    {
        "id":"tp4_l_2",
        "type":"listening",
        "romaji":"kouen",
        "moji":"公園",
        "kanji":"公園",
        "english":"park",
        "answerOption":[
            {
                "id":"tp4_l_4",
                "type":"listening",
                "moji":"会社"
            },
            {
                "id":"tp4_l_3",
                "type":"listening",
                "moji":"会社の近くのお店"
            },
            {
                "id":"tp4_l_1",
                "type":"listening",
                "moji":"スーパー"
            },
            {
                "id":"tp4_l_2",
                "type":"listening",
                "moji":"公園"
            }
        ],
        "audio":"t4_listening_2.mp3"
    },
    {
        "id":"tp4_l_3",
        "type":"listening",
        "romaji":"densha de ikimasu",
        "moji":"電車で行きます",
        "kanji":"電車で行きます",
        "english":"I'll go by train",
        "answerOption":[
            {
                "id":"tp4_l_4",
                "type":"listening",
                "moji":"タクシーで行きます"
            },
            {
                "id":"tp4_l_2",
                "type":"listening",
                "moji":"バスで行きます"
            },
            {
                "id":"tp4_l_3",
                "type":"listening",
                "moji":"電車で行きます"
            },
            {
                "id":"tp4_l_1",
                "type":"listening",
                "moji":"歩いていきます"
            }
        ],
        "audio":"t4_listening_3.mp3"
    }
]

export const reading = [
    {
		"id":"tp4_r_1",
		"type":"reading",
		"moji":"昨日、天気がよかったですから、公園へ行きました。そして、新しい時計を買いたいですから、デパートへ買い物に行きました。人がたくさんいました。アメリカ人もいました。英語で話しました。デパートで昼ごはんを食べました。おいしかったです。",
		"questionOption":{
			"moji":"Q：どうしてデパートへ行きましたか。",
			"kanji":"Q：どうしてデパートへ行きましたか。"
		},
		"answerOption":[
			{
				"id":"tp4_r_3",
				"type":"reading",
				"moji":"天気がよかったですから"
			},
			{
				"id":"tp4_r_1",
				"type":"reading",
				"moji":"新しい時計を買いたいですから"
			},
			{
				"id":"tp4_r_2",
				"type":"reading",
				"moji":"英語で話したいですから"
			},
			{
				"id":"tp4_r_4",
				"type":"reading",
				"moji":"ごはんを食べたいですから"
			}
		],
		"audio":""
	},
	{
		"id":"tp4_r_2",
		"type":"reading",
		"moji":"わたしは1年に1回、旅行に行きます。去年、沖縄へ行きました。今年、京都へ行きました。桜を見たかったですから。京都で写真をたくさんとりました。来年は、北海道へ行きたいです。雪を見たいですから。",
		"questionOption":{
			"moji":"Q：いつ桜を見ましたか",
			"kanji":"Q：いつ桜を見ましたか"
		},
		"answerOption":[
			{
				"id":"tp4_r_1",
				"type":"reading",
				"moji":"1年"
			},
			{
				"id":"tp4_r_3",
				"type":"reading",
				"moji":"去年"
			},
			{
				"id":"tp4_r_2",
				"type":"reading",
				"moji":"今年"
			},
			{
				"id":"tp4_r_4",
				"type":"reading",
				"moji":"来年"
			}
		],
		"audio":""
	},
	{
		"id":"tp4_r_3",
		"type":"reading",
		"moji":"わたしは日本語がよくわかりませんから、毎日、学校で日本語を勉強します。1週間に3回、5時から9時まで喫茶店で働きます。1日に4時間です。この店はコーヒーがおしいですから、いつもにぎやかです。お店は朝10時から夜9時までです。",
		"questionOption":{
			"moji":"Q：1週間に何時間、働きますか。",
			"kanji":"Q：1週間に何時間、働きますか。"
		},
		"answerOption":[
			{
				"id":"tp4_r_1",
				"type":"reading",
				"moji":"4時間"
			},
			{
				"id":"tp4_r_2",
				"type":"reading",
				"moji":"8時間"
			},
			{
				"id":"tp4_r_3",
				"type":"reading",
				"moji":"12時間"
			},
			{
				"id":"tp4_r_4",
				"type":"reading",
				"moji":"16時間"
			}
		],
		"audio":""
	},
]