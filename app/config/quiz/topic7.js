export const vocabulary = [
    {
        "id":"tp7_v_1",
        "type":"vocabulary",
        "romaji":"abimasu",
        "moji":"あびます",	
        "english":"to bathe, to shower",
        "audio":"t7_abimasu.mp3"
    },
    {
        "id":"tp7_v_2",
        "type":"vocabulary",
        "romaji":"arukimasu",
        "moji":"あるきます",	
        "english":"to walk",
        "audio":"t7_arukimasu.mp3"
    },
    {
        "id":"tp7_v_3",
        "type":"vocabulary",
        "romaji":"īmasu",
        "moji":"いいます",	
        "english":"to say",
        "audio":"t7_iimasu.mp3"
    },
    {
        "id":"tp7_v_4",
        "type":"vocabulary",
        "romaji":"isogashī",
        "moji":"いそがしい",	
        "english":"busy, irritated",
        "audio":"t7_isogashii.mp3"
    },
    {
        "id":"tp7_v_5",
        "type":"vocabulary",
        "romaji":"uta",
        "moji":"うた",	
        "english":"song",
        "audio":"t7_uta.mp3"
    },
    {
        "id":"tp7_v_6",
        "type":"vocabulary",
        "romaji":"utaimasu",
        "moji":"うたいます",	
        "english":"to sing",
        "audio":"t7_utaimasu.mp3"
    },
    {
        "id":"tp7_v_7",
        "type":"vocabulary",
        "romaji":"umaremasu",
        "moji":"うまれます",	
        "english":"to be born",
        "audio":"t7_umaremasu.mp3"
    },
    {
        "id":"tp7_v_8",
        "type":"vocabulary",
        "romaji":"onaji",
        "moji":"おなじ",	
        "english":"same",
        "audio":"t7_onaji.mp3"
    },
    {
        "id":"tp7_v_9",
        "type":"vocabulary",
        "romaji":"o furo",
        "moji":"おふろ",	
        "english":"Bath",
        "audio":"t7_ofuro.mp3"
    },
    {
        "id":"tp7_v_10",
        "type":"vocabulary",
        "romaji":"o bentō",
        "moji":"おべんとう",	
        "english":"boxed lunch",
        "audio":"t7_obentou.mp3"
    },
    {
        "id":"tp7_v_11",
        "type":"vocabulary",
        "romaji":"ka shimasu",
        "moji":"かします",	
        "english":"to lend/to loan​",
        "audio":"t7_kashimasu.mp3"
    },
    {
        "id":"tp7_v_12",
        "type":"vocabulary",
        "romaji":"kaze",
        "moji":"かぜ",	
        "english":"Wind",
        "audio":"t7_kaze.mp3"
    },
    {
        "id":"tp7_v_13",
        "type":"vocabulary",
        "romaji":"kaze",
        "moji":"かぜ",	
        "english":"a cold",
        "audio":"t7_kaze.mp3"
    },
    {
        "id":"tp7_v_14",
        "type":"vocabulary",
        "romaji":"kado",
        "moji":"かど",	
        "english":"a corner",
        "audio":"t7_kado.mp3"
    },
    {
        "id":"tp7_v_15",
        "type":"vocabulary",
        "romaji":"kawa",
        "moji":"かわ",	
        "english":"River",
        "audio":"t7_kawa.mp3"
    },
    {
        "id":"tp7_v_16",
        "type":"vocabulary",
        "romaji":"gitā",
        "moji":"ギター",	
        "english":"Guitar",
        "audio":"t7_gita.mp3"
    },
    {
        "id":"tp7_v_17",
        "type":"vocabulary",
        "romaji":"kusuri",
        "moji":"くすり",	
        "english":"medicine",
        "audio":"t7_kusuri.mp3"
    },
    {
        "id":"tp7_v_18",
        "type":"vocabulary",
        "romaji":"kō sate n",
        "moji":"こうさてん",	
        "english":"intersection",
        "audio":"t7_kousaten.mp3"
    },
    {
        "id":"tp7_v_19",
        "type":"vocabulary",
        "romaji":"saifu",
        "moji":"さいふ",	
        "english":"Wallet",
        "audio":"t7_saifu.mp3"
    },
    {
        "id":"tp7_v_20",
        "type":"vocabulary",
        "romaji":"jikan",
        "moji":"じかん",	
        "english":"Time",
        "audio":"t7_jikan.mp3"
    },
    {
        "id":"tp7_v_21",
        "type":"vocabulary",
        "romaji":"shawā",
        "moji":"シャワー",	
        "english":"Shower",
        "audio":"t7_shawa.mp3"
    },
    {
        "id":"tp7_v_22",
        "type":"vocabulary",
        "romaji":"sentaku shimasu",
        "moji":"せんたくします",	
        "english":"to launder/to do washing",
        "audio":"t7_sentakushimasu.mp3"
    },
    {
        "id":"tp7_v_23",
        "type":"vocabulary",
        "romaji":"sōji shimasu",
        "moji":"そうじします",	
        "english":"to clean, to sweep",
        "audio":"t7_soujishimasu.mp3"
    },
    {
        "id":"tp7_v_24",
        "type":"vocabulary",
        "romaji":"daijōbu",
        "moji":"だいじょうぶ",	
        "english":"all right",
        "audio":"t7_daijoubu.mp3"
    },
    {
        "id":"tp7_v_25",
        "type":"vocabulary",
        "romaji":"takushī",
        "moji":"タクシー",	
        "english":"taxi",
        "audio":"t7_takushi.mp3"
    },
    {
        "id":"tp7_v_26",
        "type":"vocabulary",
        "romaji":"dandan",
        "moji":"だんだん",	
        "english":"gradually",
        "audio":"t7_dandan.mp3"
    },
    {
        "id":"tp7_v_27",
        "type":"vocabulary",
        "romaji":"chika tetsu",
        "moji":"ちかてつ",	
        "english":"subway",
        "audio":"t7_chikatetsu.mp3"
    },
    {
        "id":"tp7_v_28",
        "type":"vocabulary",
        "romaji":"tsugi",
        "moji":"つぎ",	
        "english":"next",
        "audio":"t7_tsugi.mp3"
    },
    {
        "id":"tp7_v_29",
        "type":"vocabulary",
        "romaji":"dekakemasu",
        "moji":"でかけます",	
        "english":"to go out",
        "audio":"t7_dekakemasu.mp3"
    },
    {
        "id":"tp7_v_30",
        "type":"vocabulary",
        "romaji":"de masu",
        "moji":"でます",	
        "english":"to appear,to leave",
        "audio":"t7_demasu.mp3"
    },
    {
        "id":"tp7_v_31",
        "type":"vocabulary",
        "romaji":"naraimasu",
        "moji":"ならいます",	
        "english":"to learn",
        "audio":"t7_naraimasu.mp3"
    },
    {
        "id":"tp7_v_32",
        "type":"vocabulary",
        "romaji":"noborimasu",
        "moji":"のぼります",	
        "english":"to climb",
        "audio":"t7_noborimasu.mp3"
    },
    {
        "id":"tp7_v_33",
        "type":"vocabulary",
        "romaji":"hairimasu",
        "moji":"はいります",	
        "english":"to enter, to contain",
        "audio":"t7_hairimasu.mp3"
    },
    {
        "id":"tp7_v_34",
        "type":"vocabulary",
        "romaji":"hako",
        "moji":"はこ",	
        "english":"box",
        "audio":"t7_hako.mp3"
    },
    {
        "id":"tp7_v_35",
        "type":"vocabulary",
        "romaji":"ha",
        "moji":"は",	
        "english":"tooth",
        "audio":"t7_ha.mp3"
    },
    {
        "id":"tp7_v_36",
        "type":"vocabulary",
        "romaji":"hashi",
        "moji":"はし",	
        "english":"bridge",
        "audio":"t7_hashi.mp3"
    },
    {
        "id":"tp7_v_37",
        "type":"vocabulary",
        "romaji":"hajimete",
        "moji":"はじめて",	
        "english":"for the first time",
        "audio":"t7_hajimete.mp3"
    },
    {
        "id":"tp7_v_38",
        "type":"vocabulary",
        "romaji":"hashirimasu",
        "moji":"はしります",	
        "english":"to run",
        "audio":"t7_hashirimasu.mp3"
    },
    {
        "id":"tp7_v_39",
        "type":"vocabulary",
        "romaji":"byōin",
        "moji":"びょういん",	
        "english":"hospital",
        "audio":"t7_byouin.mp3"
    },
    {
        "id":"tp7_v_40",
        "type":"vocabulary",
        "romaji":"byōki",
        "moji":"びょうき",	
        "english":"illness",
        "audio":"t7_byouki.mp3"
    },
    {
        "id":"tp7_v_41",
        "type":"vocabulary",
        "romaji":"mada",
        "moji":"まだ",	
        "english":"yet, still",
        "audio":"t7_mada.mp3"
    },
    {
        "id":"tp7_v_42",
        "type":"vocabulary",
        "romaji":"migakimasu",
        "moji":"みがきます",	
        "english":"to brush teeth, to polish",
        "audio":"t7_migakimasu.mp3"
    },
    {
        "id":"tp7_v_43",
        "type":"vocabulary",
        "romaji":"michi",
        "moji":"みち",	
        "english":"street",
        "audio":"t7_michi.mp3"
    },
    {
        "id":"tp7_v_44",
        "type":"vocabulary",
        "romaji":"mō",
        "moji":"もう",	
        "english":"already",
        "audio":"t7_mou.mp3"
    },
    {
        "id":"tp7_v_45",
        "type":"vocabulary",
        "romaji":"yama",
        "moji":"やま",	
        "english":"",
        "audio":"t7_yama.mp3"
    },
    {
        "id":"tp7_v_46",
        "type":"vocabulary",
        "romaji":"renshū shimasu",
        "moji":"れんしゅうします",	
        "english":"to practice",
        "audio":"t7_renshuushimasu.mp3"
    },
    {
        "id":"tp7_v_47",
        "type":"vocabulary",
        "romaji":"watarimasu",
        "moji":"わたります",	
        "english":"to go across",
        "audio":"t7_watarimasu.mp3"
    },
]

export const kanji = [
    {
        "id":"tp7_k_1",
        "type":"kanji",
        "romaji":"karada",
        "moji":"からだ",
        "kanji":"体",
        "english":"body",
        "audio":"t7_karada.mp3"
    },
    {
        "id":"tp7_k_2",
        "type":"kanji",
        "romaji":"me",
        "moji":"め",
        "kanji":"目",
        "english":"eye",
        "audio":"t7_me.mp3"
    },
    {
        "id":"tp7_k_3",
        "type":"kanji",
        "romaji":"kuchi",
        "moji":"くち",
        "kanji":"口",
        "english":"mouth",
        "audio":"t7_kuchi.mp3"
    },
    {
        "id":"tp7_k_4",
        "type":"kanji",
        "romaji":"mimi",
        "moji":"みみ",
        "kanji":"耳",
        "english":"ear",
        "audio":"t7_mimi.mp3"
    },
    {
        "id":"tp7_k_5",
        "type":"kanji",
        "romaji":"te",
        "moji":"て",
        "kanji":"手",
        "english":"hand",
        "audio":"t7_te.mp3"
    },
    {
        "id":"tp7_k_6",
        "type":"kanji",
        "romaji":"ashi",
        "moji":"あし",
        "kanji":"足",
        "english":"foot",
        "audio":"t7_ashi.mp3"
    },
    {
        "id":"tp7_k_7",
        "type":"kanji",
        "romaji":"chikara",
        "moji":"ちから",
        "kanji":"力",
        "english":"power",
        "audio":"t7_chikara.mp3"
    },
    {
        "id":"tp7_k_8",
        "type":"kanji",
        "romaji":"ushi",
        "moji":"うし",
        "kanji":"牛",
        "english":"cow",
        "audio":"t7_ushi.mp3"
    },
    {
        "id":"tp7_k_9",
        "type":"kanji",
        "romaji":"uma",
        "moji":"うま",
        "kanji":"馬",
        "english":"horse",
        "audio":"t7_uma.mp3"
    },
    {
        "id":"tp7_k_10",
        "type":"kanji",
        "romaji":"sakana",
        "moji":"さかな",
        "kanji":"魚",
        "english":"fish",
        "audio":"t7_sakana.mp3"
    },
    {
        "id":"tp7_k_11",
        "type":"kanji",
        "romaji":"kai",
        "moji":"かい",
        "kanji":"貝",
        "english":"shell",
        "audio":"t7_kai.mp3"
    },
    {
        "id":"tp7_k_12",
        "type":"kanji",
        "romaji":"niku",
        "moji":"にく",
        "kanji":"肉",
        "english":"meat",
        "audio":"t7_niku.mp3"
    },
    {
        "id":"tp7_k_13",
        "type":"kanji",
        "romaji":"Shiro",
        "moji":"しろ",
        "kanji":"白",
        "english":"white",
        "audio":"t7_shiro.mp3"
    },
    {
        "id":"tp7_k_14",
        "type":"kanji",
        "romaji":"kuro",
        "moji":"くろ",
        "kanji":"黒",
        "english":"black",
        "audio":"t7_kuro.mp3"
    },
    {
        "id":"tp7_k_15",
        "type":"kanji",
        "romaji":"koku/ kuni",
        "moji":"こく/くに",
        "kanji":"国",
        "english":"country",
        "audio":"t7_koku_kuni.mp3"
    },
]

export const grammar = [
    {
        "id":"tp7_g_1",
        "type":"grammar",
        "romaji":"mō shukudai o shimashita.",
        "moji":"もう宿題をしました。",
        "kanji":"もう宿題をしました。",
        "english":"I have  already done my homework.",
        "audio":"tp7_g_1.mp3"
    },
    {
        "id":"tp7_g_2",
        "type":"grammar",
        "romaji":"mō Hokkaidō e ikimashita.",
        "moji":"もう北海道へ行きました。",
        "kanji":"もう北海道へ行きました。",
        "english":"I have  already gone to Hokkaido.",
        "audio":"tp7_g_2.mp3"
    },
    {
        "id":"tp7_g_3",
        "type":"grammar",
        "romaji":"mō hirugohan o tabemashita.",
        "moji":"もう昼ご飯を食べました。",
        "kanji":"もう昼ご飯を食べました。",
        "english":"I have  already  eaten lunch.",
        "audio":"tp7_g_3.mp3"
    },
    {
        "id":"tp7_g_4",
        "type":"grammar",
        "romaji":"mō hiragana o oboemashita.",
        "moji":"もうひらがなを覚えました。",
        "kanji":"もうひらがなを覚えました。",
        "english":"I have  already memorized to Hiragana.",
        "audio":"tp7_g_4.mp3"
    },
    {
        "id":"tp7_g_5",
        "type":"grammar",
        "romaji":"mō chiketto o kaimashita.",
        "moji":"もうチケットを買いました。",
        "kanji":"もうチケットを買いました。",
        "english":"I have already  bought the tickets.",
        "audio":"tp7_g_5.mp3"
    },
    {
        "id":"tp7_g_6",
        "type":"grammar",
        "romaji":"mada shukudai o shite imasen.",
        "moji":"まだ宿題をしていません。",
        "kanji":"まだ宿題をしていません。",
        "english":"I have not done my homework.",
        "audio":"tp7_g_6.mp3"
    },
    {
        "id":"tp7_g_7",
        "type":"grammar",
        "romaji":"mada Hokkaidō e itte imasen.",
        "moji":"まだ北海道へ行っていません。",
        "kanji":"まだ北海道へ行っていません。",
        "english":"I have not gone to Hokkaido.",
        "audio":"tp7_g_7.mp3"
    },
    {
        "id":"tp7_g_8",
        "type":"grammar",
        "romaji":"mada hirugohan o tabete imasen.",
        "moji":"まだ昼ご飯を食べていません。",
        "kanji":"まだ昼ご飯を食べていません。",
        "english":"I have not eaten lunch yet.",
        "audio":"tp7_g_8.mp3"
    },
    {
        "id":"tp7_g_9",
        "type":"grammar",
        "romaji":"mada hiragana o oboete imasen.",
        "moji":"まだひらがなを覚えていません。",
        "kanji":"まだひらがなを覚えていません。",
        "english":"I have not memorized to Hiragana.",
        "audio":"tp7_g_9.mp3"
    },
    {
        "id":"tp7_g_10",
        "type":"grammar",
        "romaji":"mada chiketto o katte imasen.",
        "moji":"まだチケットを買っていません。",
        "kanji":"まだチケットを買っていません。",
        "english":"I have not bought the tickets.",
        "audio":"tp7_g_10.mp3"
    },
    {
        "id":"tp7_g_11",
        "type":"grammar",
        "romaji":"shawā o abita ato de, asagohan o tabemasu.",
        "moji":"シャワーを浴びたあとで、朝ご飯を食べます。",
        "kanji":"シャワーを浴びたあとで、朝ご飯を食べます。",
        "english":"I'll eat breakfast after taking a shower.",
        "audio":"tp7_g_11.mp3"
    },
    {
        "id":"tp7_g_12",
        "type":"grammar",
        "romaji":"oyoida ato de, bīru o nomimasu.",
        "moji":"泳いだあとで、ビールを飲みます。",
        "kanji":"泳いだあとで、ビールを飲みます。",
        "english":"I'll drink a beer after swinmming.",
        "audio":"tp7_g_12.mp3"
    },
    {
        "id":"tp7_g_13",
        "type":"grammar",
        "romaji":"kaetta ato de, te o araimasu.",
        "moji":"帰ったあとで、手を洗います。",
        "kanji":"帰ったあとで、手を洗います。",
        "english":"I'll wash my hands after going home.",
        "audio":"tp7_g_13.mp3"
    },
    {
        "id":"tp7_g_14",
        "type":"grammar",
        "romaji":"sōji shita ato de, de kakemasu.",
        "moji":"掃除したあとで、でかけます。",
        "kanji":"掃除したあとで、でかけます。",
        "english":"I'll go out after cleaning.",
        "audio":"tp7_g_14.mp3"
    },
    {
        "id":"tp7_g_15",
        "type":"grammar",
        "romaji":"shokuji no ato de, kōhī o nomimasu.",
        "moji":"食事のあとで、コーヒーを飲みます。",
        "kanji":"食事のあとで、コーヒーを飲みます。",
        "english":"I'll drink a coffee after eating.",
        "audio":"tp7_g_15.mp3"
    },
    {
        "id":"tp7_g_16",
        "type":"grammar",
        "romaji":"Tōkyō de, tomodachi ni attari, kaimono shi tari shimashita.",
        "moji":"東京で、友達に会ったり、買い物したりしました。",
        "kanji":"東京で、友達に会ったり、買い物したりしました。",
        "english":"In Tokyo, I meet my friend, go shopping and so on.",
        "audio":"tp7_g_16.mp3"
    },
    {
        "id":"tp7_g_17",
        "type":"grammar",
        "romaji":"nichiyō hi wa ie de hon o yon dari ongaku o kī tari shimasu.",
        "moji":"日曜日は家で本を読んだり音楽を聞いたりします。",
        "kanji":"日曜日は家で本を読んだり音楽を聞いたりします。",
        "english":"On Sunday, I read a book, listen music and so on at home.",
        "audio":"tp7_g_17.mp3"
    },
    {
        "id":"tp7_g_18",
        "type":"grammar",
        "romaji":"yasumi no hi, terebi o mi tari bīru o non dari shimasu.",
        "moji":"休みの日、テレビを見たりビールを飲んだりします。",
        "kanji":"休みの日、テレビを見たりビールを飲んだりします。",
        "english":"On days off, I watch TV, drink beer and so on.",
        "audio":"tp7_g_18.mp3"
    },
    {
        "id":"tp7_g_19",
        "type":"grammar",
        "romaji":"kinō, sōji shi tari sentaku shi tari shimashita.",
        "moji":"きのう、掃除したり洗濯したりしました。",
        "kanji":"きのう、掃除したり洗濯したりしました。",
        "english":"Yesterday, I cleaned my room, do my washing and so on.",
        "audio":"tp7_g_19.mp3"
    },
    {
        "id":"tp7_g_20",
        "type":"grammar",
        "romaji":"kuni de kazoku to hanashi tari, tomodachi to ason dari shitaidesu.",
        "moji":"国で家族と話したり、友達と遊んだりしたいです。",
        "kanji":"国で家族と話したり、友達と遊んだりしたいです。",
        "english":"In my country, I want to talk with my family, play with my friends and so on.",
        "audio":"tp7_g_20.mp3"
    },
    {
        "id":"tp7_g_21",
        "type":"grammar",
        "romaji":"aruki nagara jūsu o nomimasu.",
        "moji":"歩きながらジュースを飲みます。",
        "kanji":"歩きながらジュースを飲みます。",
        "english":"I'll drink juice while walking.",
        "audio":"tp7_g_21.mp3"
    },
    {
        "id":"tp7_g_22",
        "type":"grammar",
        "romaji":"ongaku o kiki nagara sanpo shimasu.",
        "moji":"音楽を聞きながら散歩します。",
        "kanji":"音楽を聞きながら散歩します。",
        "english":"I'll take a walk while listening to music.",
        "audio":"tp7_g_22.mp3"
    },
    {
        "id":"tp7_g_23",
        "type":"grammar",
        "romaji":"shinbun o yomi nagara, asagohan o tabemasu.",
        "moji":"新聞を読みながら、朝ご飯を食べます。",
        "kanji":"新聞を読みながら、朝ご飯を食べます。",
        "english":"I'll eat breakfast while reading the newspaper.",
        "audio":"tp7_g_23.mp3"
    },
    {
        "id":"tp7_g_24",
        "type":"grammar",
        "romaji":"hon o mi nagara, ryōri o tsukurimasu.",
        "moji":"本を見ながら、料理を作ります。",
        "kanji":"本を見ながら、料理を作ります。",
        "english":"I'll cook while looking at a book.",
        "audio":"tp7_g_24.mp3"
    },
    {
        "id":"tp7_g_25",
        "type":"grammar",
        "romaji":"gitā o hiki nagara, utaimasu.",
        "moji":"ギターを弾きながら、歌います。",
        "kanji":"ギターを弾きながら、歌います。",
        "english":"I'll sing a song while playing guitar.",
        "audio":"tp7_g_25.mp3"
    },
    {
        "id":"tp7_g_26",
        "type":"grammar",
        "romaji":"Nippon e kuru mae ni, san kagetsu Nippon go o benkyō shimashita.",
        "moji":"日本へ来る前に、三か月日本語を勉強しました。",
        "kanji":"日本へ来る前に、三か月日本語を勉強しました。",
        "english":"I studied Japanese for three months before I came to Japan.",
        "audio":"tp7_g_26.mp3"
    },
    {
        "id":"tp7_g_27",
        "type":"grammar",
        "romaji":"kuni e kaeru mae ni, mō ichi do tomodachi ni aimasu.",
        "moji":"国へ帰る前に、もう一度友達に会います。",
        "kanji":"国へ帰る前に、もう一度友達に会います。",
        "english":"I'll meet my friend before I go back home.",
        "audio":"tp7_g_27.mp3"
    },
    {
        "id":"tp7_g_28",
        "type":"grammar",
        "romaji":"gohan o taberu mae ni, kusuri o nomimasu.",
        "moji":"ごはんを食べる前に、薬を飲みます。",
        "kanji":"ごはんを食べる前に、薬を飲みます。",
        "english":"I'll take medicine before eating.",
        "audio":"tp7_g_28.mp3"
    },
    {
        "id":"tp7_g_29",
        "type":"grammar",
        "romaji":"benkyō no mae ni, bangohan o tabemasu.",
        "moji":"勉強の前に、晩ご飯を食べます。",
        "kanji":"勉強の前に、晩ご飯を食べます。",
        "english":"I'll eat dinner before studying.",
        "audio":"tp7_g_29.mp3"
    },
    {
        "id":"tp7_g_30",
        "type":"grammar",
        "romaji":"ichi kagetsu mae ni, Nippon e kimashita.",
        "moji":"一か月前に、日本へ来ました。",
        "kanji":"一か月前に、日本へ来ました。",
        "english":"I came to Japan a month ago.",
        "audio":"tp7_g_30.mp3"
    },
]

export const listening = [
    {
        "id":"tp7_l_1",
        "type":"listening",
        "romaji":"駅の前の喫茶店",
        "moji":"駅の前の喫茶店",
        "kanji":"駅の前の喫茶店",
        "english":"駅の前の喫茶店",
        "answerOption":[
            {
                "id":"tp7_l_2",
                "type":"listening",
                "moji":"駅の出口"
            },
            {
                "id":"tp7_l_1",
                "type":"listening",
                "moji":"駅の前の喫茶店"
            },
            {
                "id":"tp7_l_3",
                "type":"listening",
                "moji":"駅の近くのスーパー"
            },
            {
                "id":"tp7_l_4",
                "type":"listening",
                "moji":"山"
            }
        ],
        "audio":"t7_listening_1.mp3"
    },
    {
        "id":"tp7_l_2",
        "type":"listening",
        "romaji":"映画を見ました",
        "moji":"映画を見ました",
        "kanji":"映画を見ました",
        "english":"映画を見ました",
        "answerOption":[
            {
                "id":"tp7_l_1",
                "type":"listening",
                "moji":"家を掃除しました"
            },
            {
                "id":"tp7_l_3",
                "type":"listening",
                "moji":"料理を作りました"
            },
            {
                "id":"tp7_l_2",
                "type":"listening",
                "moji":"映画を見ました"
            },
            {
                "id":"tp7_l_4",
                "type":"listening",
                "moji":"友達にメールを書きました"
            }
        ],
        "audio":"t7_listening_2.mp3"
    },
    {
        "id":"tp7_l_1",
        "type":"listening",
        "romaji":"かばんを部屋に持って行きます",
        "moji":"かばんを部屋に持って行きます",
        "kanji":"かばんを部屋に持って行きます",
        "english":"かばんを部屋に持って行きます",
        "answerOption":[
            {
                "id":"tp7_l_1",
                "type":"listening",
                "moji":"昼ご飯を食べます。"
            },
            {
                "id":"tp7_l_3",
                "type":"listening",
                "moji":"かばんを部屋に持って行きます"
            },
            {
                "id":"tp7_l_2",
                "type":"listening",
                "moji":"海へ遊びに行きます"
            },
            {
                "id":"tp7_l_4",
                "type":"listening",
                "moji":"旅行会社の人に聞きます。"
            }
        ],
        "audio":"t7_listening_3.mp3"
    }
]

export const reading = [
    {
        "id":"tp7_r_1",
        "type":"reading",
        "moji":"私は去年から、学校で月曜日から金曜日まで、日本語の勉強をしています。初めはとても難しかったですが、今は少し日本語が上手になりました。学校では、文法を習ったり、日本人の先生と日本語で話したりしています。　家では、漢字を書いたり、日本語のCDを聞いたりしています。日本語を勉強した後で、日本人の友だちを作って、いっしょに富士山に登ったり、すしを食べたりしたいです。",
        "questionOption":{
            "moji":"Q：日本語を勉強してから、どうしたいですか。",
            "kanji":"Q：日本語を勉強してから、どうしたいですか。"
        },
        "answerOption":[
            {
                "id":"ans_r_5",
                "type":"reading",
                "moji":"学校で勉強したいです"
            },
            {
                "id":"ans_r_6",
                "type":"reading",
                "moji":"日本語で話したいです"
            },
            {
                "id":"ans_r_7",
                "type":"reading",
                "moji":"漢字を習いたいです"
            },
            {
                "id":"ans_r_1",
                "type":"reading",
                "moji":"日本人の友だちがほしいです"
            },
        ],
        "audio":""	
    },
    {
        "id":"tp7_r_2",
        "type":"reading",
        "moji":"今日、私は会社を休みました。昨日、友だちと昼ごはんを食べた後、おなかがいたくなりました。今日もまだいたいからです。昨日はとても暑かったですから、友だちと話しながら、冷たいビールをたくさん飲んだり、からい料理を食べたりしました。友だちに会う前にも、アイスクリームを食べたり、ジュースを飲んだりしました。私は冷たくて、あまいものが好きですから。病院はまだ行っていません。朝、起きた後で薬を飲みました。",
        "questionOption":{
            "moji":"Q：どうして会社を休みましたか。",
            "kanji":"Q：どうして会社を休みましたか。"
        },
        "answerOption":[
            {
                "id":"ans_r_2",
                "type":"reading",
                "moji":"おなかがいたいからです"
            },
            {
                "id":"ans_r_6",
                "type":"reading",
                "moji":"病院へ行くからです"
            },
            {
                "id":"ans_r_7",
                "type":"reading",
                "moji":"暑いからです"
            },
            {
                "id":"ans_r_8",
                "type":"reading",
                "moji":"薬を飲んだからです"
            },
        ],
        "audio":""	
    },
    {
        "id":"tp7_r_3",
        "type":"reading",
        "moji":"私は月曜日から金曜日まで働いています。休みの日は、友だちの家でいっしょにDVDを見たり、部屋をそうじしたりしています。友だちの家は、少し遠いです。電車で30分かかります。1か月に1回、プールで泳いでいます。家からプールは近いです。いつもプールで泳いだ後、スーパーで買い物をして家へ帰ります。スーパーは家とプールの間にあります。会社はプールより遠いです。",
        "questionOption":{
            "moji":"Q：どこが家から一番近いですか。",
            "kanji":"Q：どこが家から一番近いですか。"
        },
        "answerOption":[
            {
                "id":"ans_r_5",
                "type":"reading",
                "moji":"会社"
            },
            {
                "id":"ans_r_6",
                "type":"reading",
                "moji":"友だちの家"
            },
            {
                "id":"ans_r_7",
                "type":"reading",
                "moji":"プール"
            },
            {
                "id":"ans_r_3",
                "type":"reading",
                "moji":"スーパー"
            },
        ],
        "audio":""	
    },
]